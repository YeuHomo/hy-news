<template>
	<view @click="open">
		...
		<!-- 多图卡片 -->
		<view v-if="item.mode==='column'" class="listcard moda-column">
			...
			<view class="listcard-content">
				...
				<view class="listcard-image">
					<view v-for="(it,index) in item.cover" v-if="index<3" :key="it" class="listcard-image__item">
		...
		<!-- 大图模式 -->
		<view v-if="item.mode==='image'" class="listcard moda-image">
			<view class="listcard-image">
				<image :src="item.cover[0]" mode="aspectFill"></image>
			</view>
			<view class="listcard-content">
